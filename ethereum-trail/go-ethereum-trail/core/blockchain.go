...
func (bc *BlockChain) ResetWithGenesisBlock(genesis *types.Block) error {
	...
	trail.DefaultStore().UpdateRelayEpochFromBlockTime(genesis.Time(), genesis.Time())
...
func (bc *BlockChain) writeHeadBlock(block *types.Block) {
	...
	if bc.genesisBlock != nil {
		trail.DefaultStore().UpdateRelayEpochFromBlockTime(bc.genesisBlock.Time(), block.Time())
	}
}
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, statedb *state.StateDB) error {
	...
	trail.DefaultStore().RecordBlockEvidenceWithReceipts(block.Hash(), block.NumberU64(), block.Transactions(), receipts, block.BaseFee(), block.Coinbase())
