...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*ProcessResult, error) {
	...
	if _, err := trail.DefaultStore().ApplyCommittedSettlementRecords(
		trail.SettlementRootFromExtra(header.Extra),
		block.TrailSettlementRecords(),
		statedb,
		true,
	); err != nil {
		return nil, fmt.Errorf("failed to apply trail settlement: %w", err)
	}
