...
func TestTrailSettlementRecordsSurviveBlockCopies(t *testing.T) {
	header := &Header{
	...
	records := []TrailSettlementRecord{{
		FinalizedEpoch: 1,
		Epoch:          0,
		Role:           "relay",
		ValidatorIndex: 7,
		PayoutAddress:  common.Address{0x01}.Hex(),
		AmountWei:      "42",
		ID:             "test-record",
	}}
	block := NewBlockWithHeader(header).WithBody(Body{
		TrailSettlementRecords: records,
	})

	if got := len(block.TrailSettlementRecords()); got != 1 {
		t.Fatalf("records lost in WithBody: got %d, want 1", got)
	}
	if got := len(block.WithWitness(&ExecutionWitness{}).TrailSettlementRecords()); got != 1 {
		t.Fatalf("records lost in WithWitness: got %d, want 1", got)
	}
	if got := len(block.WithSeal(header).TrailSettlementRecords()); got != 1 {
		t.Fatalf("records lost in WithSeal: got %d, want 1", got)
	}
}
