...
func TestStoreCreatesOriginMetadata(t *testing.T) {
	secret := blst.KeyGen([]byte("trail prompt14 deterministic test key material"))
	t.Setenv("TRAIL_RELAY_VALIDATOR_INDEX", "2")
	t.Setenv("TRAIL_RELAY_PRIVATE_KEY", "0x"+hex.EncodeToString(secret.Serialize()))
	t.Setenv("TRAIL_CHAIN_ID", "7032030")
	t.Setenv("TRAIL_RELAY_EPOCH", "7")
...
func TestStoreDerivesRelayEpochFromCanonicalBlockTime(t *testing.T) {
	secret := blst.KeyGen([]byte("trail dynamic relay epoch test key material"))
...
func TestStoreInjectsDevnetPathEvidence(t *testing.T) {
	...
	for i := uint64(0); i < 3; i++ {
		secret := blst.KeyGen([]byte("trail devnet injected path evidence relay key " + string(rune('0'+i))))
	...
	t.Setenv("TRAIL_RELAY_PRIVATE_REGISTRY_JSON", string(raw))
	t.Setenv("TRAIL_CHAIN_ID", "7032030")
	t.Setenv("TRAIL_RELAY_EPOCH", "7")
...
func TestStoreDisabledWithoutRelayKey(t *testing.T) {
	t.Setenv("TRAIL_RELAY_VALIDATOR_INDEX", "2")
	t.Setenv("TRAIL_RELAY_PRIVATE_KEY", "")
	store := NewStoreFromEnv()
	if store.Enabled() {
...
func TestStoreSelectsRelayKeyFromPrivateRegistry(t *testing.T) {
	secret0 := blst.KeyGen([]byte("trail prompt14 private registry key 0"))
	secret1 := blst.KeyGen([]byte("trail prompt14 private registry key 1"))
	...
	t.Setenv("TRAIL_RELAY_PRIVATE_REGISTRY_JSON", string(raw))
	t.Setenv("TRAIL_RELAY_NODE_INDEX", "1")
...
func TestStoreMapsPeerByRemoteAddress(t *testing.T) {
	store := testStore(0, blst.KeyGen([]byte("trail prompt14 address map local")))
...
func TestStoreCompletesSignedEdge(t *testing.T) {
	senderKey := blst.KeyGen([]byte("trail prompt14 sender key material"))
	receiverKey := blst.KeyGen([]byte("trail prompt14 receiver key material"))
...
func TestStoreMetadataBatchForPeerIsConcurrentSafe(t *testing.T) {
	senderKey := blst.KeyGen([]byte("trail concurrent sender key material"))
	receiverKey := blst.KeyGen([]byte("trail concurrent receiver key material"))
...
func TestStoreRejectsBadSenderSignature(t *testing.T) {
	senderKey := blst.KeyGen([]byte("trail prompt14 sender key material bad"))
	receiverKey := blst.KeyGen([]byte("trail prompt14 receiver key material bad"))
	sender := testStore(0, senderKey)
	receiver := testStore(1, receiverKey)
	sender.peers["peer-b"] = relayIdentity{ValidatorIndex: 1, Address: fallbackRelayAddress(1)}
	pub0 := new(blst.P2Affine).From(senderKey).Compress()
...
func TestStoreRecordsBlockEvidence(t *testing.T) {
	secret := blst.KeyGen([]byte("trail prompt14 block evidence key"))
...
func TestStoreRecordsCommittedFeeInput(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	secret := blst.KeyGen([]byte("trail prompt23 fee evidence key"))
	store := testStore(0, secret)
	tx := types.NewTx(&types.LegacyTx{
		Nonce:    1,
		To:       &common.Address{0x42},
...
func TestStoreRecordsIrrecoverableBaseFeeCost(t *testing.T) {
	secret := blst.KeyGen([]byte("trail frozen v1 base fee cost key"))
	store := testStore(0, secret)
	tx := types.NewTx(&types.LegacyTx{
		Nonce:    1,
		To:       &common.Address{0x42},
...
func TestStoreAppliesSubmittedSettlementOnce(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	t.Setenv("TRAIL_SETTLEMENT_MUTATION", "1")
	store := testStore(0, blst.KeyGen([]byte("trail prompt23 settlement execution key")))
...
func TestStoreDeduplicatesSettlementByEvidenceEpoch(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	t.Setenv("TRAIL_SETTLEMENT_MUTATION", "1")
	store := testStore(0, blst.KeyGen([]byte("trail duplicate finalized settlement key")))
...
func TestStoreSettlementMutationDefaultsToNoop(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	store := testStore(0, blst.KeyGen([]byte("trail prompt23 settlement noop key")))
...
func TestApplyCommittedSettlementRecordsRequiresBlockRecordsWhenMutating(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	t.Setenv("TRAIL_SETTLEMENT_MUTATION", "1")
	store := testStore(0, blst.KeyGen([]byte("trail prompt43 settlement records required key")))
...
func TestApplyCommittedSettlementRecordsErrorsWhenMissingEverywhere(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	t.Setenv("TRAIL_SETTLEMENT_MUTATION", "1")
	store := testStore(0, blst.KeyGen([]byte("trail prompt43 missing settlement records key")))
	memoryDB := rawdb.NewMemoryDatabase()
	stateDB, err := state.New(types.EmptyRootHash, state.NewDatabase(triedb.NewDatabase(memoryDB, nil), nil))
	if err != nil {
		t.Fatal(err)
...
func TestApplyCommittedSettlementRecordsErrorsWhenEscrowInsufficient(t *testing.T) {
	t.Setenv("TRAIL_FEE_ESCROW", "1")
	t.Setenv("TRAIL_SETTLEMENT_MUTATION", "1")
	store := testStore(0, blst.KeyGen([]byte("trail prompt43 insufficient escrow key")))
