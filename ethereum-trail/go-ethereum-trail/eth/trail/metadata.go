...
var (
	...
	txMetadataCreatedMeter   = metrics.NewRegisteredMeter("trail/tx/metadata/created", nil)
	txMetadataForwardMeter   = metrics.NewRegisteredMeter("trail/tx/metadata/forwarded", nil)
	txMetadataReceivedMeter  = metrics.NewRegisteredMeter("trail/tx/metadata/received", nil)
	txMetadataSignedOrigin   = metrics.NewRegisteredMeter("trail/tx/metadata/signed/origin", nil)
	txMetadataSignedSender   = metrics.NewRegisteredMeter("trail/tx/metadata/signed/sender", nil)
	txMetadataSignedRecv     = metrics.NewRegisteredMeter("trail/tx/metadata/signed/receiver", nil)
	txMetadataVerifiedOK     = metrics.NewRegisteredMeter("trail/tx/metadata/verified/sender/ok", nil)
	txMetadataVerifiedFail   = metrics.NewRegisteredMeter("trail/tx/metadata/verified/sender/fail", nil)
	txMetadataInvalidMeter   = metrics.NewRegisteredMeter("trail/tx/metadata/invalid", nil)
	txMetadataBytesMeter     = metrics.NewRegisteredMeter("trail/tx/metadata/bytes", nil)
	txPathLengthGauge        = metrics.NewRegisteredGauge("trail/tx/path/length", nil)
	blockEvidenceMeter       = metrics.NewRegisteredMeter("trail/block/evidence/recorded", nil)
	blockEvidenceTxGauge     = metrics.NewRegisteredGauge("trail/block/evidence/txs", nil)
	settlementSubmittedMeter = metrics.NewRegisteredMeter("trail/settlement/submitted", nil)
	settlementAppliedMeter   = metrics.NewRegisteredMeter("trail/settlement/applied", nil)
	settlementSkippedMeter   = metrics.NewRegisteredMeter("trail/settlement/skipped", nil)
)

func FeeEscrowEnabled() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("TRAIL_FEE_ESCROW"))) {
...
func SettlementMutationEnabled() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("TRAIL_SETTLEMENT_MUTATION"))) {
	case "1", "true", "yes", "on":
		return true
	default:
		return false
...
// TxMetadata is the devnet-only TRAIL transaction propagation sidecar.
...
// DefaultStore returns the process-wide TRAIL propagation metadata store.
...
func NewStoreFromEnv() *Store {
	store := &Store{
		chainID:               getenvUint64("TRAIL_CHAIN_ID", DefaultChainID),
		dynamicEpoch:          getenvBool("TRAIL_DYNAMIC_RELAY_EPOCH", false),
		secondsPerSlot:        getenvUint64("TRAIL_SECONDS_PER_SLOT", DefaultSecondsPerSlot),
		slotsPerEpoch:         getenvUint64("TRAIL_SLOTS_PER_EPOCH", DefaultSlotsPerEpoch),
		maxBytes:              int(getenvUint64("TRAIL_TX_METADATA_MAX_BYTES", DefaultMaxBytes)),
	...
	store.epoch.Store(getenvUint64("TRAIL_RELAY_EPOCH", 0))
	store.loadRelayRegistryFromEnv()
	store.loadPeerRegistryFromEnv()
	store.loadPrivateRelayRegistryFromEnv()

	validatorRaw := strings.TrimSpace(os.Getenv("TRAIL_RELAY_VALIDATOR_INDEX"))
	privateRaw := strings.TrimSpace(os.Getenv("TRAIL_RELAY_PRIVATE_KEY"))
...
func (s *Store) InjectPathEvidence(hash common.Hash, validators []uint64) (TxMetadata, error) {
	if s == nil {
		return nil, fmt.Errorf("trail store is nil")
	}
	if len(validators) < 2 {
		return nil, fmt.Errorf("trail path requires at least origin and receiver")
	}
	secrets, err := trailPrivateRelaySecretsFromEnv(s)
	if err != nil {
		return nil, err
	}
	...
	if len(encoded) > s.maxBytes {
		return nil, fmt.Errorf("trail metadata too large: %d", len(encoded))
	}
...
func (s *Store) SubmitSettlement(payload SettlementPayload) (int, error) {
	if s == nil {
		return 0, fmt.Errorf("nil trail store")
	}
	if strings.TrimSpace(payload.Domain) == "" {
		payload.Domain = "TRAIL_FEE_SETTLEMENT_V1"
	}
	if payload.Domain != "TRAIL_FEE_SETTLEMENT_V1" {
...
func (s *Store) SettlementRecordsForRoot(root common.Hash) []types.TrailSettlementRecord {
	...
	records := make([]types.TrailSettlementRecord, 0, len(payload.Records))
	for _, record := range payload.Records {
		records = append(records, types.TrailSettlementRecord{
...
func (s *Store) ApplyCommittedSettlementRecords(root common.Hash, records []types.TrailSettlementRecord, statedb *state.StateDB, markExecuted bool) (int, error) {
	return s.applyCommittedSettlement(root, records, statedb, markExecuted)
}

func (s *Store) applyCommittedSettlement(root common.Hash, blockRecords []types.TrailSettlementRecord, statedb *state.StateDB, markExecuted bool) (int, error) {
	...
	if hasBlockRecords {
		...
		if computed != root {
			return 0, fmt.Errorf("trail settlement root mismatch: committed %s computed %s", root, computed)
		}
		payload = &converted
	} else {
		if SettlementMutationEnabled() {
			return 0, fmt.Errorf("missing block-carried trail settlement records for committed root %s", root)
		}
		s.lock.RLock()
		payload = s.settlementsByRoot[root]
		s.lock.RUnlock()
	...
	if !ok {
		return 0, fmt.Errorf("invalid trail settlement amount for root %s", root)
	}
	if total.IsZero() {
		return 0, fmt.Errorf("zero trail settlement total for root %s", root)
	}
	if statedb.GetBalance(FeeEscrowAddress).Cmp(total) < 0 {
		return 0, fmt.Errorf("insufficient trail escrow balance for root %s: have %s need %s", root, statedb.GetBalance(FeeEscrowAddress), total)
	}
...
func SettlementRootFromExtra(extra []byte) common.Hash {
	...
	if !bytes.Equal(extra[:len(trailSettlementExtraPrefix)], trailSettlementExtraPrefix) {
		return common.Hash{}
	}
...
func settlementPayloadFromBlockRecords(records []types.TrailSettlementRecord) (SettlementPayload, error) {
	if len(records) == 0 {
		return SettlementPayload{}, fmt.Errorf("empty trail settlement records")
	}
	...
	for _, record := range records {
		if record.FinalizedEpoch != payload.FinalizedEpoch || record.Epoch != payload.Epoch {
			return SettlementPayload{}, fmt.Errorf("mixed trail settlement epochs in block records")
		}
...
func settlementPayloadRoot(payload SettlementPayload) common.Hash {
	hasher := sha256.New()
	hasher.Write([]byte("TRAIL_FEE_SETTLEMENT_ROOT_V1"))
	hasher.Write([]byte{0})
...
var trailSettlementExtraPrefix = []byte{'T', 'S', 'S', '1'}

func settlementPayloadCommitment(root common.Hash) common.Hash {
	var out [common.HashLength]byte
	copy(out[:], trailSettlementExtraPrefix)
	copy(out[len(trailSettlementExtraPrefix):], root[:common.HashLength-len(trailSettlementExtraPrefix)])
...
func (s *Store) loadRelayRegistryFromEnv() {
	raw := strings.TrimSpace(os.Getenv("TRAIL_RELAY_PUBLIC_REGISTRY_JSON"))
	if raw == "" {
		raw = readOptionalFile(os.Getenv("TRAIL_RELAY_PUBLIC_REGISTRY"))
	}
	if raw == "" {
		return
	}
...
func (s *Store) loadPeerRegistryFromEnv() {
	raw := strings.TrimSpace(os.Getenv("TRAIL_RELAY_PEER_REGISTRY_JSON"))
	if raw == "" {
		raw = readOptionalFile(os.Getenv("TRAIL_RELAY_PEER_REGISTRY"))
	}
	if raw == "" {
		return
	}
...
func (s *Store) loadPrivateRelayRegistryFromEnv() {
	raw := strings.TrimSpace(os.Getenv("TRAIL_RELAY_PRIVATE_REGISTRY_JSON"))
	if raw == "" {
		raw = readOptionalFile(os.Getenv("TRAIL_RELAY_PRIVATE_REGISTRY"))
	}
	if raw == "" {
		return
	}
	...
	explicitNodeIndex, hasExplicitNodeIndex := getenvOptionalUint64("TRAIL_RELAY_NODE_INDEX")
...
func trailPrivateRelaySecretsFromEnv(s *Store) (map[uint64]*blst.SecretKey, error) {
	raw := strings.TrimSpace(os.Getenv("TRAIL_RELAY_PRIVATE_REGISTRY_JSON"))
	if raw == "" {
		raw = readOptionalFile(os.Getenv("TRAIL_RELAY_PRIVATE_REGISTRY"))
	}
	if raw == "" {
		return nil, fmt.Errorf("missing TRAIL_RELAY_PRIVATE_REGISTRY_JSON")
	}
	var registry privateRelayRegistryFile
	if err := json.Unmarshal([]byte(raw), &registry); err != nil {
		return nil, err
...
func blockEvidenceRoot(blockHash common.Hash, blockNumber uint64, records []blockSignatureRecord, txs []BlockTransactionEvidence) []byte {
	var buf [8]byte
	digest := sha256.New()
	digest.Write([]byte("TRAIL_BLOCK_EVIDENCE_V1"))
