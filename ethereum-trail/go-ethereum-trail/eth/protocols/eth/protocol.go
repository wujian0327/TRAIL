...
type NewPooledTransactionHashesPacket struct {
	...
	Sizes     []uint32
	Hashes    []common.Hash
	Trail []TrailTxMetadata `rlp:"optional"`
}

// TrailTxMetadata is a devnet-only transaction propagation metadata sidecar.
// It must not be included in the signed Ethereum transaction bytes.
type TrailTxMetadata []byte
