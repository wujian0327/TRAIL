...
func (p *Peer) sendPooledTransactionHashes(hashes []common.Hash, types []byte, sizes []uint32) error {
	// Mark all the transactions as known, but ensure we don't overflow our limits
	p.knownTxs.Add(hashes...)
	return p2p.Send(p.rw, NewPooledTransactionHashesMsg, NewPooledTransactionHashesPacket{
		Types:     types,
		Sizes:     sizes,
		Hashes:    hashes,
		Trail: metadataBatch(hashes, p.ID(), p.RemoteAddr().String()),
	})
}
...
func metadataBatch(hashes []common.Hash, peerID string, remoteAddr string) []TrailTxMetadata {
	store := trail.DefaultStore()
	if !store.Enabled() {
		return nil
	}
	raw := store.MetadataBatchForPeerAddress(hashes, peerID, remoteAddr)
	if len(raw) == 0 {
		return nil
	}
	batch := make([]TrailTxMetadata, len(raw))
	for i := range raw {
		batch[i] = TrailTxMetadata(raw[i])
	}
	return batch
}
