...
import (
	...
	"github.com/ethereum/go-ethereum/eth/trail"
...
func handleNewPooledTransactionHashes(backend Backend, msg Decoder, peer *Peer) error {
	...
	if len(ann.Trail) > 0 && len(ann.Trail) != len(ann.Hashes) {
		trail.DefaultStore().RecordInbound(nil, nil)
		return fmt.Errorf("NewPooledTransactionHashes: invalid trail metadata len %v for hashes %v", len(ann.Trail), len(ann.Hashes))
	}
	recordInboundMetadata(ann.Hashes, ann.Trail)
...
func handlePooledTransactions(backend Backend, msg Decoder, peer *Peer) error {
	...
	if len(txs.Trail) > 0 && len(txs.Trail) != len(txs.PooledTransactionsResponse) {
		trail.DefaultStore().RecordInbound(nil, nil)
		return fmt.Errorf("PooledTransactions: invalid trail metadata len %v for txs %v", len(txs.Trail), len(txs.PooledTransactionsResponse))
	}
	if len(txs.Trail) > 0 {
		...
		recordInboundMetadata(hashes, txs.Trail)
	}
...
func recordInboundMetadata(hashes []common.Hash, batch []TrailTxMetadata) {
	if len(batch) == 0 {
		return
	}
	raw := make([]trail.TxMetadata, len(batch))
	for i := range batch {
		raw[i] = trail.TxMetadata(batch[i])
	}
	trail.DefaultStore().RecordInbound(hashes, raw)
}
