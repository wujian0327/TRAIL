...
func (miner *Miner) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	settlementRoot := trail.SettlementRootFromExtra(work.header.Extra)
	if settlementRoot != (common.Hash{}) {
		body.TrailSettlementRecords = trail.DefaultStore().SettlementRecordsForRoot(settlementRoot)
		if trail.SettlementMutationEnabled() && len(body.TrailSettlementRecords) == 0 {
			return &newPayloadResult{err: fmt.Errorf("missing block-carried trail settlement records for pending root %s", settlementRoot)}
		}
	}
	...
	if _, err := trail.DefaultStore().ApplyCommittedSettlementRecords(settlementRoot, body.TrailSettlementRecords, work.state, false); err != nil {
		return &newPayloadResult{err: err}
	}
	...
	trail.DefaultStore().RecordBlockEvidenceWithReceipts(block.Hash(), block.NumberU64(), block.Transactions(), work.receipts, block.BaseFee(), block.Coinbase())
...
func (miner *Miner) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	if root := trail.DefaultStore().PendingSettlementRoot(); root != (common.Hash{}) {
		header.Extra = trail.SettlementExtraFromRoot(root)
	}
