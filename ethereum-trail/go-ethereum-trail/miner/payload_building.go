...
func (payload *Payload) Resolve() *engine.ExecutionPayloadEnvelope {
	...
	if payload.full != nil {
		...
		attachTrailBlockEvidence(envelope, payload.full, payload.fullReceipts)
	...
	attachTrailBlockEvidence(envelope, payload.empty, nil)
...
func attachTrailBlockEvidence(envelope *engine.ExecutionPayloadEnvelope, block *types.Block, receipts []*types.Receipt) {
	if envelope == nil || block == nil {
		return
	}
	evidence := trail.DefaultStore().BuildBlockEvidence(
		block.Hash(),
		block.NumberU64(),
		block.Transactions(),
		receipts,
	...
	if err != nil {
		log.Warn("Failed to encode TRAIL payload evidence", "block", block.Hash(), "err", err)
		return
	}
	envelope.TrailBlockEvidence = encoded
}
