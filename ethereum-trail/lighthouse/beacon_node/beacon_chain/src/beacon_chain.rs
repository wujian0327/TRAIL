...
use crate::graffiti_calculator::{
    GraffitiCalculator, GraffitiSettings, trail_tx_evidence_records_from_json,
    trail_tx_evidence_root_from_json,
};
...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    fn complete_partial_beacon_block<Payload: AbstractExecPayload<T::EthSpec>>(
        ...
        let trail_timing_started = Instant::now();
        let trail_block_evidence = block_contents
            .as_ref()
            .and_then(|contents| contents.trail_block_evidence());
        let trail_has_evidence = trail_block_evidence.is_some();
        let trail_root_started = Instant::now();
        let trail_evidence_root =
            trail_tx_evidence_root_from_json(trail_block_evidence.clone())
                .unwrap_or_else(Hash256::zero);
        let trail_root_ms = trail_root_started.elapsed().as_millis();
        let trail_records_started = Instant::now();
        let trail_evidence_records = trail_tx_evidence_records_from_json::<T::EthSpec>(
            trail_block_evidence,
            &self.spec,
        );
        let trail_records_ms = trail_records_started.elapsed().as_millis();
        info!(
            slot = slot.as_u64(),
            proposer_index = proposer_index,
            has_evidence = trail_has_evidence,
            evidence_records = trail_evidence_records.len(),
            evidence_root = %trail_evidence_root,
            root_ms = trail_root_ms,
            records_ms = trail_records_ms,
            total_ms = trail_timing_started.elapsed().as_millis(),
            slot_elapsed_ms = self
                .slot_clock
                .millis_from_current_slot_start()
                .map(|d| d.as_millis()),
            "TRAIL block evidence packed"
        );
            ...
            BeaconState::Base(_) => (
                BeaconBlock::Base(BeaconBlockBase {
                    ...
                    body: BeaconBlockBodyBase {
                        ...
                        graffiti,
                        trail_evidence_root,
                        trail_evidence_records: trail_evidence_records.clone(),
