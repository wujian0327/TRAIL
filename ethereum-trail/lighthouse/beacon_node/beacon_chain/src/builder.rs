...
fn make_genesis_block<E: EthSpec>(
    ...
    // TRAIL devnet extends `BeaconBlockBody`, so a genesis state produced by the
    // stock ethereum-package generator has a `latest_block_header.body_root` for the
    // stock empty body. Normalize it to the custom empty-body root before hashing and
    // storing genesis, otherwise fork choice will look for a stock-root genesis block
    // while the store contains the custom-root genesis block.
    genesis_state.latest_block_header_mut().body_root = block.body_root();
