...
use types::{
    ...
    block::BlockImportSource, execution::ExecPayload,
};
...
const TOPOS_SIDECAR_MARKER: &[u8] = b"TRAIL";

fn transaction_contains_trail_marker<E: EthSpec>(block: &SignedBeaconBlock<E>) -> bool {
    block.message().execution_payload().is_ok_and(|payload| {
        payload.transactions().is_some_and(|transactions| {
            transactions.iter().any(|transaction| {
                transaction
                    .iter()
                    .copied()
                    .collect::<Vec<_>>()
                    .windows(TOPOS_SIDECAR_MARKER.len())
                    .any(|window| window == TOPOS_SIDECAR_MARKER)
            })
        })
    })
}
...
impl<T: BeaconChainTypes> NetworkBeaconProcessor<T> {
    ...
    async fn process_gossip_unverified_block(
        ...
        let trail_sidecar_marker = transaction_contains_trail_marker(block.as_ref());
        let trail_sidecar_marker_label = if trail_sidecar_marker {
            "present"
        } else {
            "absent"
        };
        let peer_client_kind = peer_client.kind.as_ref();
        // Log metrics to track delay from other nodes on the network.
        ...
        let block_root = if let Ok(verified_block) = &verification_result {
            ...
            let block_root_label = verified_block.block_root.to_string();
            let slot_label = verified_block.block.slot().to_string();
            let proposer_index_label = verified_block.block.message().proposer_index().to_string();
            metrics::inc_counter_vec(
                &metrics::TRAIL_GOSSIP_BLOCK_ARRIVALS_TOTAL,
                &[peer_client_kind, trail_sidecar_marker_label],
            );
            metrics::set_gauge_vec(
                &metrics::TRAIL_GOSSIP_BLOCK_ARRIVAL_DELAY_MILLISECONDS,
                &[peer_client_kind, trail_sidecar_marker_label],
                block_delay.as_millis() as i64,
            );
            metrics::set_gauge_vec(
                &metrics::TRAIL_GOSSIP_BLOCK_INFO,
                &[
                    &block_root_label,
                    &slot_label,
                    &proposer_index_label,
                    peer_client_kind,
                    trail_sidecar_marker_label,
                ],
                1,
            );
            verified_block.block_root
        ...
        let verified_block = match verification_result {
            Ok(verified_block) => {
                    ...
                    .and_then(|now| now.checked_sub(seen_duration))
                {
                    ...
                    metrics::set_gauge_vec(
                        &metrics::TRAIL_GOSSIP_BLOCK_VERIFICATION_DELAY_MILLISECONDS,
                        &[peer_client_kind, trail_sidecar_marker_label],
                        duration.as_millis() as i64,
                    );
                }
