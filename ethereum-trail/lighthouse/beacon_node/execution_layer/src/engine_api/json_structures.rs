...
#[derive(Debug, PartialEq, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct JsonTrailSettlementRecord {
...
fn trail_settlement_role_code(role: &str) -> u8 {
...
fn trail_settlement_role_name(role: u8) -> String {
    match role {
        1 => "relay",
        2 => "burned",
        _ => "proposer",
...
impl From<JsonTrailSettlementRecord> for TrailSettlementRecord {
    fn from(record: JsonTrailSettlementRecord) -> Self {
        let amount_wei =
        ...
        Self {
            ...
            epoch: record.epoch,
            role: trail_settlement_role_code(&record.role),
...
impl From<TrailSettlementRecord> for JsonTrailSettlementRecord {
    fn from(record: TrailSettlementRecord) -> Self {
        ...
        Self {
            finalized_epoch: record.finalized_epoch,
            epoch: record.epoch,
            role: trail_settlement_role_name(record.role),
            validator_index: record.validator_index,
            payout_address,
...
fn trail_settlements_from_json<E: EthSpec>(
    records: VariableList<JsonTrailSettlementRecord, E::MaxValidatorsPerCommittee>,
) -> Result<VariableList<TrailSettlementRecord, E::MaxValidatorsPerCommittee>, ssz_types::Error>
{
    VariableList::new(records.into_iter().map(Into::into).collect())
}

fn trail_settlements_to_json<E: EthSpec>(
    records: VariableList<TrailSettlementRecord, E::MaxValidatorsPerCommittee>,
) -> Result<
    VariableList<JsonTrailSettlementRecord, E::MaxValidatorsPerCommittee>,
    ssz_types::Error,
> {
    VariableList::new(records.into_iter().map(Into::into).collect())
}
...
impl<E: EthSpec> From<ExecutionPayloadBellatrix<E>> for JsonExecutionPayloadBellatrix<E> {
    fn from(payload: ExecutionPayloadBellatrix<E>) -> Self {
        JsonExecutionPayloadBellatrix {
            ...
            trail_settlement_records: trail_settlements_to_json::<E>(
                payload.trail_settlement_records,
            )
            .expect("trail settlement list length should be preserved"),
        }
    }
}
...
impl<E: EthSpec> From<JsonExecutionPayloadBellatrix<E>> for ExecutionPayloadBellatrix<E> {
    fn from(payload: JsonExecutionPayloadBellatrix<E>) -> Self {
        ExecutionPayloadBellatrix {
            ...
            block_hash: payload.block_hash,
            transactions: payload.transactions,
            trail_settlement_records: trail_settlements_from_json::<E>(
                payload.trail_settlement_records,
            )
            .expect("trail settlement list length should be preserved"),
        }
...
pub struct JsonGetPayloadResponse<E: EthSpec> {
    ...
    #[serde(default)]
    pub trail_block_evidence: Option<serde_json::Value>,
}
        ...
        match json_get_payload_response {
            JsonGetPayloadResponse::Bellatrix(response) => {
                Ok(GetPayloadResponse::Bellatrix(GetPayloadResponseBellatrix {
                    ...
                    trail_block_evidence: response.trail_block_evidence,
                }))
            }
...
impl<E: EthSpec> TryFrom<JsonExecutionPayloadBodyV1<E>> for ExecutionPayloadBodyV1<E> {
    ...
    fn try_from(value: JsonExecutionPayloadBodyV1<E>) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            trail_settlement_records: trail_settlements_from_json::<E>(
                value.trail_settlement_records,
            )?,
        })
    }
}
...
mod tests {
    ...
    use types::{
        ...
        TrailSettlementRecord, WithdrawalRequest,
    };
    ...
    #[test]
    fn trail_settlement_records_roundtrip_in_execution_payload_json() {
        let settlement = TrailSettlementRecord {
            finalized_epoch: 3,
            epoch: 1,
            role: 1,
            validator_index: 7,
        ...
        let records = value
            .get("trailSettlementRecords")
            .and_then(|value| value.as_array())
            .expect("trail settlement records should use Engine API camelCase");
        ...
        let decoded_records = match decoded_payload {
            ExecutionPayload::Bellatrix(payload) => payload.trail_settlement_records,
            ExecutionPayload::Capella(payload) => payload.trail_settlement_records,
            ExecutionPayload::Deneb(payload) => payload.trail_settlement_records,
            ExecutionPayload::Electra(payload) => payload.trail_settlement_records,
            ExecutionPayload::Fulu(payload) => payload.trail_settlement_records,
            ExecutionPayload::Gloas(payload) => payload.trail_settlement_records,
        };
