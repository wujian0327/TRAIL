...
pub const TRAIL_FIXED_POINT_SCALE: u64 = 1_000_000_000;
pub const TRAIL_GRAFFITI_EVIDENCE_PREFIX: &str = "TPS1:";
...
impl ChainSpec {
    ...
    /// Returns true if the devnet-only TRAIL consensus skeleton is active.
    ///
    /// The current minimal TRAIL fork gates config/state skeleton access and
    /// proposer selection. It does not alter rewards, block processing, or
    /// BeaconState SSZ.
    pub fn is_trail_enabled_at_epoch(&self, epoch: Epoch) -> bool {
        self.trail_config.is_enabled_at_epoch(epoch)
    }
    ...
    pub fn mainnet() -> Self {
        Self {
            ...
            trail_config: TrailConfig::disabled(),
...
/// Consensus configuration for the devnet-only TRAIL fork.
///
...
pub struct TrailConfig {
    ...
    pub trail_fork_epoch: Option<Epoch>,
    #[serde(default = "default_trail_eta_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub eta_scaled: u64,
    #[serde(default = "default_trail_bonus_cap_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub bonus_cap_scaled: u64,
    #[serde(default = "default_trail_score_ema_beta_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub score_ema_beta_scaled: u64,
    #[serde(default = "default_trail_score_saturation_k_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub score_saturation_k_scaled: u64,
    #[serde(
        default = "default_trail_score_target_depth",
        alias = "SCORE_INITIAL_DEPTH"
    )]
    #[serde(with = "serde_utils::quoted_u64")]
    pub score_target_depth: u64,
    #[serde(default = "default_trail_score_cost_reference_wei")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub score_cost_reference_wei: u64,
    #[serde(default = "default_trail_score_floor_kappa_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub score_floor_kappa_scaled: u64,
    #[serde(default = "default_trail_bonus_zeta_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub bonus_zeta_scaled: u64,
    #[serde(default = "default_trail_reward_settlement_depth")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub reward_settlement_depth: u64,
    #[serde(default = "default_trail_max_path_evidence_len")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub max_path_evidence_len: u64,
    #[serde(default = "default_trail_max_path_evidence_bytes")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub max_path_evidence_bytes: u64,
    #[serde(default = "default_trail_max_paths_per_block")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub max_paths_per_block: u64,
    #[serde(default = "default_trail_proposer_fee_ratio_scaled")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub proposer_fee_ratio_scaled: u64,
    #[serde(default = "default_trail_evidence_finality_depth")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub evidence_finality_depth: u64,
    #[serde(default = "default_trail_score_activation_delay_epochs")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub score_activation_delay_epochs: u64,
    #[serde(default = "default_trail_evidence_work_limit")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub evidence_work_limit: u64,
    #[serde(default = "default_trail_challenge_work_limit")]
    #[serde(with = "serde_utils::quoted_u64")]
    pub challenge_work_limit: u64,
    #[serde(default)]
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub score_fixture: Vec<TrailScoreFixture>,
}

impl TrailConfig {
    pub fn disabled() -> Self {
        Self {
            trail_fork_epoch: None,
            eta_scaled: default_trail_eta_scaled(),
            bonus_cap_scaled: default_trail_bonus_cap_scaled(),
            score_ema_beta_scaled: default_trail_score_ema_beta_scaled(),
            score_saturation_k_scaled: default_trail_score_saturation_k_scaled(),
            score_target_depth: default_trail_score_target_depth(),
            score_cost_reference_wei: default_trail_score_cost_reference_wei(),
            score_floor_kappa_scaled: default_trail_score_floor_kappa_scaled(),
            bonus_zeta_scaled: default_trail_bonus_zeta_scaled(),
            reward_settlement_depth: default_trail_reward_settlement_depth(),
            max_path_evidence_len: default_trail_max_path_evidence_len(),
            max_path_evidence_bytes: default_trail_max_path_evidence_bytes(),
            max_paths_per_block: default_trail_max_paths_per_block(),
            proposer_fee_ratio_scaled: default_trail_proposer_fee_ratio_scaled(),
            evidence_finality_depth: default_trail_evidence_finality_depth(),
            score_activation_delay_epochs: default_trail_score_activation_delay_epochs(),
            evidence_work_limit: default_trail_evidence_work_limit(),
            challenge_work_limit: default_trail_challenge_work_limit(),
            score_fixture: vec![],
        }
    }
    ...
    pub fn devnet_enabled_at(fork_epoch: Epoch) -> Self {
        Self {
            trail_fork_epoch: Some(fork_epoch),
            ..Self::disabled()
        }
    }

    pub fn is_disabled(&self) -> bool {
        self.trail_fork_epoch.is_none() && trail_env_epoch("TRAIL_FORK_EPOCH").is_none()
    }

    pub fn is_enabled_at_epoch(&self, epoch: Epoch) -> bool {
        self.trail_fork_epoch
            .or_else(|| trail_env_epoch("TRAIL_FORK_EPOCH"))
            .is_some_and(|fork_epoch| epoch >= fork_epoch)
    }

    pub fn evidence_finality_depth(&self) -> u64 {
        trail_env_u64("TRAIL_EVIDENCE_FINALITY_DEPTH")
            .unwrap_or(self.evidence_finality_depth)
    }

    pub fn score_activation_delay_epochs(&self) -> u64 {
        trail_env_u64("TRAIL_SCORE_ACTIVATION_DELAY_EPOCHS")
    ...
    pub fn score_for_validator_at_epoch(&self, epoch: Epoch, validator_index: usize) -> u64 {
        self.score_for_validator(validator_index).max(
            trail_evidence_scores_for_epoch(epoch, self.score_activation_delay_epochs())
                .get(&validator_index)
                .copied()
                .unwrap_or(0),
        )
    ...
    pub fn score_total_at_epoch(&self, epoch: Epoch) -> u64 {
        ...
        for (validator_index, score) in trail_env_score_fixture() {
    ...
    pub fn bonus_scaled_at_epoch_with_score_total(
        ...
        let scale = TRAIL_FIXED_POINT_SCALE as u128;
    ...
    pub fn bonus_multiplier_from_bonus_scaled(&self, bonus_scaled: u64) -> u64 {
        ...
        let eta_scaled = trail_env_u64("TRAIL_ETA_SCALED")
            .unwrap_or(self.eta_scaled)
            .min(TRAIL_FIXED_POINT_SCALE) as u128;
        let scale = TRAIL_FIXED_POINT_SCALE as u128;
    ...
    ) -> Option<TrailStateSkeleton> {
        self.is_enabled_at_epoch(epoch)
            .then_some(TrailStateSkeleton::zero_scores(epoch, validator_count))
    }
}

fn trail_env_epoch(name: &str) -> Option<Epoch> {
    trail_env_u64(name).map(Epoch::new)
}

fn trail_env_u64(name: &str) -> Option<u64> {
    env::var(name)
        .ok()
        .and_then(|value| value.trim().parse::<u64>().ok())
}

fn trail_env_score_fixture() -> Vec<(usize, u64)> {
    env::var("TRAIL_SCORE_FIXTURE")
        .ok()
        .map(|value| {
            value
                .split(',')
...
pub enum TrailEvidenceSource {
    InlineGraffiti,
    TxGossipMetadata,
}

impl TrailEvidenceSource {
...
pub struct TrailEvidenceEpochSummary {
    pub epoch: Epoch,
    pub valid_paths: u64,
...
#[derive(Debug, PartialEq, Eq, Clone, Default)]
pub struct TrailValidatorCredit {
    pub validator_index: usize,
    pub credit_scaled: u64,
}

#[derive(Debug, PartialEq, Eq, Clone, Default)]
pub struct TrailValidatorFeeSettlement {
    pub validator_index: usize,
    pub amount_wei: u64,
}

#[derive(Debug, PartialEq, Eq, Clone)]
pub struct TrailPathEvidence {
    pub tx_hash: String,
    pub path: Vec<usize>,
...
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct TrailSettledEpochScores {
    pub epoch: Epoch,
    pub score_total_scaled: u64,
    pub updates: Vec<TrailScoreUpdate>,
}

#[derive(Debug, PartialEq, Eq, Clone, Default)]
pub struct TrailCreditEpochSummary {
    pub epoch: Epoch,
    ...
    pub proposer_credits: Vec<TrailValidatorCredit>,
    pub relay_credits: Vec<TrailValidatorCredit>,
}

#[derive(Debug, PartialEq, Eq, Clone, Default)]
pub struct TrailFeeSettlementEpochSummary {
    pub epoch: Epoch,
    ...
    pub conservation_violation: bool,
    pub proposer_settlements: Vec<TrailValidatorFeeSettlement>,
    pub relay_settlements: Vec<TrailValidatorFeeSettlement>,
}

#[derive(Debug, PartialEq, Eq, Clone)]
pub enum TrailEvidenceRecordOutcome {
    Disabled,
    NoEvidence,
    Valid {
        epoch: Epoch,
        source: TrailEvidenceSource,
        receiver: usize,
        score_updates: Vec<(usize, u64)>,
        summary: TrailEvidenceEpochSummary,
    },
    Invalid {
        epoch: Epoch,
        summary: TrailEvidenceEpochSummary,
...
struct TrailEvidenceEpochRuntime {
    valid_paths: u64,
    invalid_paths: u64,
    ...
    credit_records: Vec<TrailCreditRecordRuntime>,
...
impl TrailEvidenceEpochRuntime {
    fn summary(&self, epoch: Epoch, _bonus_cap_scaled: u64) -> TrailEvidenceEpochSummary {
    ...
    fn credit_summary(&self, epoch: Epoch) -> TrailCreditEpochSummary {
        ...
        TrailCreditEpochSummary {
            ...
            proposer_credits: sorted_trail_credits(&self.proposer_credits),
            relay_credits: sorted_trail_credits(&self.relay_credits),
        }
    }

    fn fee_settlement_summary(&self, epoch: Epoch) -> TrailFeeSettlementEpochSummary {
        let credit = self.credit_summary(epoch);
        TrailFeeSettlementEpochSummary {
            epoch,
            ...
            proposer_settlements: credit
                .proposer_credits
                .into_iter()
                .map(|credit| TrailValidatorFeeSettlement {
...
#[derive(Debug, Clone)]
struct TrailCreditRecordRuntime {
    settlement_epoch: Epoch,
    settled: bool,
}

#[derive(Debug, Default)]
struct TrailEvidenceRuntime {
    epochs: HashMap<u64, TrailEvidenceEpochRuntime>,
...
static TRAIL_EVIDENCE_RUNTIME: OnceLock<RwLock<TrailEvidenceRuntime>> = OnceLock::new();

fn trail_evidence_runtime() -> &'static RwLock<TrailEvidenceRuntime> {
    TRAIL_EVIDENCE_RUNTIME.get_or_init(|| RwLock::new(TrailEvidenceRuntime::default()))
}

pub fn reset_trail_evidence_runtime() {
    if let Ok(mut runtime) = trail_evidence_runtime().write() {
        *runtime = TrailEvidenceRuntime::default();
    }
}

pub fn decode_trail_graffiti_evidence(
    graffiti: &Graffiti,
    spec: &ChainSpec,
    ...
    if let Some(path_csv) = raw.strip_prefix(TRAIL_GRAFFITI_EVIDENCE_PREFIX) {
        return parse_trail_validator_path(path_csv, spec).map(Some);
    }

    Ok(None)
}

fn parse_trail_validator_path(path_csv: &str, spec: &ChainSpec) -> Result<Vec<usize>, String> {
    if path_csv.is_empty() {
        return Err("empty TRAIL path".to_string());
    }
    ...
    validate_trail_validator_path(&path, spec)?;
    Ok(path)
}

fn validate_trail_validator_path(path: &[usize], spec: &ChainSpec) -> Result<(), String> {
    if path.is_empty() {
        return Err("empty TRAIL path".to_string());
    }
    if path.len() > spec.trail_config.max_path_evidence_len as usize {
        return Err(format!(
            "TRAIL path length {} exceeds max {}",
            path.len(),
            spec.trail_config.max_path_evidence_len
        ));
    }
    ...
    for validator_index in path {
        if !seen.insert(*validator_index) {
            return Err(format!(
                "repeated validator identity in TRAIL path: {validator_index}"
            ));
        }
    }
...
pub fn record_trail_graffiti_evidence(
    epoch: Epoch,
    proposer_index: usize,
    graffiti: &Graffiti,
    spec: &ChainSpec,
) -> TrailEvidenceRecordOutcome {
    if !spec.is_trail_enabled_at_epoch(epoch) {
        return TrailEvidenceRecordOutcome::Disabled;
    }

    let decoded = decode_trail_graffiti_evidence_record(graffiti, spec);
    let Ok(mut runtime) = trail_evidence_runtime().write() else {
        return TrailEvidenceRecordOutcome::Disabled;
    };
    let epoch_runtime = runtime.epochs.entry(epoch.as_u64()).or_default();

    match decoded {
        Ok(Some(decoded)) => record_trail_path(
            epoch_runtime,
            epoch,
            decoded.source,
            None,
            decoded.path,
            TRAIL_FIXED_POINT_SCALE,
            0,
            proposer_index,
            spec,
        ),
        Ok(None) => TrailEvidenceRecordOutcome::NoEvidence,
        Err(TrailEvidenceDecodeError::Invalid) => {
            epoch_runtime.invalid_paths = epoch_runtime.invalid_paths.saturating_add(1);
            TrailEvidenceRecordOutcome::Invalid {
                epoch,
                summary: epoch_runtime.summary(epoch, spec.trail_config.bonus_cap_scaled),
            }
        }
    }
}

pub fn record_trail_tx_gossip_metadata_evidence(
    epoch: Epoch,
    proposer_index: usize,
    paths: Vec<TrailPathEvidence>,
    spec: &ChainSpec,
) -> Vec<TrailEvidenceRecordOutcome> {
    if !spec.is_trail_enabled_at_epoch(epoch) {
        return vec![TrailEvidenceRecordOutcome::Disabled];
    }
    if paths.is_empty() {
        return vec![TrailEvidenceRecordOutcome::NoEvidence];
    }

    let Ok(mut runtime) = trail_evidence_runtime().write() else {
        return vec![TrailEvidenceRecordOutcome::Disabled];
    };
    let max_paths = spec.trail_config.max_paths_per_block as usize;
    let mut outcomes = Vec::new();
    for path in paths.into_iter().take(max_paths) {
        if validate_trail_validator_path(&path.path, spec).is_err() {
            let epoch_runtime = runtime.epochs.entry(epoch.as_u64()).or_default();
            epoch_runtime.invalid_paths = epoch_runtime.invalid_paths.saturating_add(1);
            outcomes.push(TrailEvidenceRecordOutcome::Invalid {
                epoch,
                summary: epoch_runtime.summary(epoch, spec.trail_config.bonus_cap_scaled),
            });
            continue;
        ...
        if runtime.accepted_tx_hashes.contains(&path.tx_hash) {
            ...
            outcomes.push(TrailEvidenceRecordOutcome::DuplicateTransaction {
                epoch,
                source: TrailEvidenceSource::TxGossipMetadata,
        ...
        let (outcome, accepted) = {
            let epoch_runtime = runtime.epochs.entry(epoch.as_u64()).or_default();
            let outcome = record_trail_path(
                epoch_runtime,
                epoch,
                TrailEvidenceSource::TxGossipMetadata,
                Some(path.tx_hash),
                path.path,
                path.fee_budget_wei,
            ...
            let accepted = matches!(outcome, TrailEvidenceRecordOutcome::Valid { .. });
            (outcome, accepted)
        };
        if accepted {
...
pub fn record_trail_invalid_tx_gossip_metadata_evidence(
    epoch: Epoch,
    count: usize,
    spec: &ChainSpec,
) -> Vec<TrailEvidenceRecordOutcome> {
    ...
    let epoch_runtime = runtime.epochs.entry(epoch.as_u64()).or_default();
    let capped_count = count.min(spec.trail_config.max_paths_per_block as usize);
...
struct DecodedTrailEvidence {
    source: TrailEvidenceSource,
    path: Vec<usize>,
}

enum TrailEvidenceDecodeError {
    Invalid,
}

fn decode_trail_graffiti_evidence_record(
    graffiti: &Graffiti,
    spec: &ChainSpec,
) -> Result<Option<DecodedTrailEvidence>, TrailEvidenceDecodeError> {
    let raw = std::str::from_utf8(&graffiti.0)
        .map_err(|_| TrailEvidenceDecodeError::Invalid)?
        .trim_matches(char::from(0))
        .trim();

    if let Some(path_csv) = raw.strip_prefix(TRAIL_GRAFFITI_EVIDENCE_PREFIX) {
        return parse_trail_validator_path(path_csv, spec)
            .map(|path| {
                Some(DecodedTrailEvidence {
                    source: TrailEvidenceSource::InlineGraffiti,
                    path,
                })
            })
            .map_err(|_| TrailEvidenceDecodeError::Invalid);
...
fn record_trail_path(
    epoch_runtime: &mut TrailEvidenceEpochRuntime,
    epoch: Epoch,
    source: TrailEvidenceSource,
    ...
    let Some(&receiver) = path.last() else {
        epoch_runtime.invalid_paths = epoch_runtime.invalid_paths.saturating_add(1);
        return TrailEvidenceRecordOutcome::Invalid {
            epoch,
            summary: epoch_runtime.summary(epoch, spec.trail_config.bonus_cap_scaled),
        };
    };
    ...
    if !epoch_runtime.accepted_tx_hashes.insert(tx_identity.clone()) {
        epoch_runtime.duplicate_receiver_proofs =
            epoch_runtime.duplicate_receiver_proofs.saturating_add(1);
        return TrailEvidenceRecordOutcome::DuplicateTransaction {
            epoch,
            source,
            tx_hash: tx_identity,
            summary: epoch_runtime.summary(epoch, spec.trail_config.bonus_cap_scaled),
    ...
    let credit_weight_scaled = trail_transaction_credit_weight_scaled(
        irrecoverable_cost_wei,
        spec.trail_config.score_cost_reference_wei,
    );
    let relay_contributions = trail_path_relay_contributions(&path, spec)
        .into_iter()
    ...
    TrailEvidenceRecordOutcome::Valid {
        epoch,
        source,
        receiver,
        score_updates,
...
pub fn trail_transaction_credit_weight_scaled(
    irrecoverable_cost_wei: u64,
    ...
    .min(TRAIL_FIXED_POINT_SCALE)
}

fn trail_path_relay_contributions(path: &[usize], spec: &ChainSpec) -> Vec<(usize, u64)> {
    ...
    let budget = trail_path_budget_scaled(edge_count, spec);
    if budget == 0 {
        return vec![];
    }

    let relay_count = edge_count.saturating_sub(1);
    let depth = spec.trail_config.score_target_depth.max(1) as u128;
    let r_scaled = (depth.saturating_mul(u128::from(TRAIL_FIXED_POINT_SCALE))
...
fn trail_path_budget_scaled(edge_count: usize, spec: &ChainSpec) -> u64 {
    if edge_count == 0 {
        return 0;
    }
    let depth = spec.trail_config.score_target_depth.max(1) as u128;
    ...
    let depth_factor = if depth >= edge_count_u128 {
        u128::from(TRAIL_FIXED_POINT_SCALE)
    } else {
        depth.saturating_mul(u128::from(TRAIL_FIXED_POINT_SCALE)) / edge_count_u128
    };
    let lambda_scaled = ((depth.saturating_mul(2).saturating_add(1))
        .saturating_mul(u128::from(TRAIL_FIXED_POINT_SCALE))
        / depth.saturating_mul(3).saturating_add(1))
    .min(u128::from(u64::MAX)) as u64;
    let decay = fixed_pow_scaled(lambda_scaled, edge_count.saturating_sub(1) as u64);
    (depth_factor.saturating_mul(u128::from(decay)) / u128::from(TRAIL_FIXED_POINT_SCALE))
        .min(u128::from(u64::MAX)) as u64
}

fn fixed_pow_scaled(base_scaled: u64, exp: u64) -> u64 {
    let mut out = u128::from(TRAIL_FIXED_POINT_SCALE);
    let base = u128::from(base_scaled);
    for _ in 0..exp {
        out = out.saturating_mul(base) / u128::from(TRAIL_FIXED_POINT_SCALE);
    }
    out.min(u128::from(u64::MAX)) as u64
}

fn record_trail_credit(
    epoch_runtime: &mut TrailEvidenceEpochRuntime,
    ...
    let proposer_credit = scaled_mul_div(
        budget,
        spec.trail_config.proposer_fee_ratio_scaled,
        TRAIL_FIXED_POINT_SCALE,
    );
    let relay_budget = budget.saturating_sub(proposer_credit);
    let relay_credits = trail_path_relay_contributions(path, spec)
        .into_iter()
        .map(|(validator_index, contribution_scaled)| {
            (
                validator_index,
    ...
    *epoch_runtime
    ...
    epoch_runtime
        .credit_records
        .push(TrailCreditRecordRuntime {
            settlement_epoch: Epoch::new(
                evidence_epoch
                    .as_u64()
                    .saturating_add(spec.trail_config.reward_settlement_depth),
            ),
            settled: false,
        });
}
...
fn sorted_trail_credits(credits: &HashMap<usize, u64>) -> Vec<TrailValidatorCredit> {
    let mut credits = credits
        .iter()
        .map(
            |(&validator_index, &credit_scaled)| TrailValidatorCredit {
                validator_index,
                credit_scaled,
            },
        )
...
pub fn trail_settle_scores_through_epoch(
    finalized_epoch: Epoch,
    validator_effective_balances: &[(usize, u64)],
    total_active_balance: u64,
    spec: &ChainSpec,
) -> Vec<TrailSettledEpochScores> {
    ...
    let Some(max_settle_epoch) = finalized_epoch
        .as_u64()
        .checked_sub(spec.trail_config.evidence_finality_depth())
    else {
        return vec![];
    };
    ...
    for epoch in epochs {
        ...
        let beta_scaled = spec
            ...
            .min(TRAIL_FIXED_POINT_SCALE);
        let one_minus_beta_scaled = TRAIL_FIXED_POINT_SCALE.saturating_sub(beta_scaled);
        ...
        for validator_index in validator_indices {
            ...
            let effective_balance = balance_by_validator
                ...
                .copied()
                .unwrap_or(0);
            let saturated_contribution_scaled = trail_saturate_contribution_scaled(
                raw_contribution_scaled,
                effective_balance,
                total_active_balance,
                spec.trail_config.score_saturation_k_scaled,
            );
        ...
        let score_total_scaled = updates
            ...
            .sum::<u64>();
        settled_epochs.push(TrailSettledEpochScores {
...
fn trail_saturate_contribution_scaled(
    ...
    scaled_mul_div(stake_share_scaled, ln_scaled, TRAIL_FIXED_POINT_SCALE)
}
...
pub fn trail_evidence_score_for_epoch(
    proposer_epoch: Epoch,
    validator_index: usize,
    activation_delay_epochs: u64,
) -> u64 {
    trail_evidence_scores_for_epoch(proposer_epoch, activation_delay_epochs)
        .get(&validator_index)
        .copied()
        .unwrap_or(0)
}

pub fn trail_evidence_scores_for_epoch(
    proposer_epoch: Epoch,
    activation_delay_epochs: u64,
) -> HashMap<usize, u64> {
...
pub fn trail_evidence_epoch_summary(
    epoch: Epoch,
    spec: &ChainSpec,
) -> TrailEvidenceEpochSummary {
    let Ok(runtime) = trail_evidence_runtime().read() else {
        return TrailEvidenceEpochSummary {
            epoch,
            ..Default::default()
        };
    };
    runtime
        .epochs
        .get(&epoch.as_u64())
        .map(|epoch_runtime| epoch_runtime.summary(epoch, spec.trail_config.bonus_cap_scaled))
        .unwrap_or(TrailEvidenceEpochSummary {
            epoch,
            ..Default::default()
        })
}

pub fn trail_settle_credits_through_epoch(finalized_epoch: Epoch) {
    let Ok(mut runtime) = trail_evidence_runtime().write() else {
        return;
    };
...
pub fn trail_credit_epoch_summaries() -> Vec<TrailCreditEpochSummary> {
    let Ok(runtime) = trail_evidence_runtime().read() else {
        return vec![];
    };
...
pub fn trail_fee_settlement_epoch_summaries() -> Vec<TrailFeeSettlementEpochSummary> {
    let Ok(runtime) = trail_evidence_runtime().read() else {
        return vec![];
    };
    let mut epochs = runtime.epochs.keys().copied().collect::<Vec<_>>();
...
pub fn trail_evidence_csv_snapshot(spec: &ChainSpec) -> String {
    let mut rows = vec![
    ...
    for epoch in epochs {
        ...
        let epoch_runtime = runtime
            ...
            .get(&epoch.as_u64())
            .expect("epoch key should exist");
        let summary = epoch_runtime.summary(epoch, spec.trail_config.bonus_cap_scaled);
...
pub fn trail_credit_csv_snapshot() -> String {
    let mut rows = vec![
    ...
    for summary in trail_credit_epoch_summaries() {
        rows.push(format!(
            "{},{},{},{},{},{},{},{},{}",
            summary.epoch,
...
pub fn trail_fee_settlement_csv_snapshot() -> String {
    let mut rows = vec![
    ...
    for summary in trail_fee_settlement_epoch_summaries() {
        rows.push(format!(
            "{},{},{},{},{},{},{},{},{}",
            summary.epoch,
...
pub struct Config {
    ...
    #[serde(default = "TrailConfig::disabled")]
    #[serde(skip_serializing_if = "TrailConfig::is_disabled")]
    pub trail_config: TrailConfig,
}
...
const fn default_trail_eta_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE / 2
}

const fn default_trail_bonus_cap_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE
}

const fn default_trail_score_ema_beta_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE * 8 / 10
}

const fn default_trail_score_saturation_k_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE
}

const fn default_trail_score_target_depth() -> u64 {
    4
}

const fn default_trail_score_cost_reference_wei() -> u64 {
    21_000_000_000_000
}

const fn default_trail_score_floor_kappa_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE
}

const fn default_trail_bonus_zeta_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE
}

const fn default_trail_reward_settlement_depth() -> u64 {
    2
}

const fn default_trail_max_path_evidence_len() -> u64 {
    16
}

const fn default_trail_max_path_evidence_bytes() -> u64 {
    65_536
}

const fn default_trail_max_paths_per_block() -> u64 {
    1024
}

const fn default_trail_proposer_fee_ratio_scaled() -> u64 {
    TRAIL_FIXED_POINT_SCALE / 2
}

const fn default_trail_evidence_finality_depth() -> u64 {
    1
}

const fn default_trail_score_activation_delay_epochs() -> u64 {
    2
}

const fn default_trail_evidence_work_limit() -> u64 {
    4096
}

const fn default_trail_challenge_work_limit() -> u64 {
    1024
}
...
impl Config {
    ...
    pub fn from_chain_spec<E: EthSpec>(spec: &ChainSpec) -> Self {
        Self {
            ...
            trail_config: spec.trail_config.clone(),
        }
    }
    ...
    pub fn apply_to_chain_spec<E: EthSpec>(&self, chain_spec: &ChainSpec) -> Option<ChainSpec> {
        ...
        let spec = ChainSpec {
            ...
            consolidation_churn_limit_quotient,
            max_per_epoch_activation_churn_limit_gloas,
            trail_config: trail_config.clone(),
...
mod yaml_tests {
    ...
    static TRAIL_TEST_LOCK: LazyLock<Mutex<()>> = LazyLock::new(|| Mutex::new(()));
    ...
    #[test]
    fn trail_config_defaults_disabled() {
        let spec = ChainSpec::mainnet();

        assert!(spec.trail_config.is_disabled());
        assert!(!spec.is_trail_enabled_at_epoch(Epoch::new(0)));
        assert_eq!(spec.trail_config.eta_scaled, 500_000_000);
        assert_eq!(
            spec.trail_config.bonus_cap_scaled,
            TRAIL_FIXED_POINT_SCALE
        );
        assert_eq!(
            spec.trail_config
    ...
    #[test]
    fn trail_config_yaml_activation() {
        let mainnet = ChainSpec::mainnet();
        let mut config = Config::from_chain_spec::<MainnetEthSpec>(&mainnet);
        config.trail_config = TrailConfig::devnet_enabled_at(Epoch::new(2));
        config.trail_config.score_fixture = vec![TrailScoreFixture {
            validator_index: 7,
            score_scaled: TRAIL_FIXED_POINT_SCALE,
        }];

        let yaml = yaml_serde::to_string(&config).expect("should serialize");
        assert!(yaml.contains("TRAIL_CONFIG"));
        ...
        assert!(!spec.is_trail_enabled_at_epoch(Epoch::new(1)));
        assert!(spec.is_trail_enabled_at_epoch(Epoch::new(2)));
        assert_eq!(
            spec.trail_config
                .zero_score_state_skeleton(Epoch::new(2), 64),
            Some(TrailStateSkeleton::zero_scores(Epoch::new(2), 64))
        );
        assert_eq!(
            spec.trail_config.score_for_validator(7),
            TRAIL_FIXED_POINT_SCALE
        );
    }

    fn trail_enabled_spec_for_tests() -> ChainSpec {
        let mainnet = ChainSpec::mainnet();
        let mut config = Config::from_chain_spec::<MainnetEthSpec>(&mainnet);
        config.trail_config = TrailConfig::devnet_enabled_at(Epoch::new(0));
        config
            .apply_to_chain_spec::<MainnetEthSpec>(&mainnet)
            .expect("trail config should apply")
    }

    #[test]
    fn trail_direct_path_has_no_relay_contribution() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        let outcomes = record_trail_tx_gossip_metadata_evidence(
            Epoch::new(0),
            1,
            vec![TrailPathEvidence {
                tx_hash: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
                    .into(),
                path: vec![0, 1],
                fee_budget_wei: TRAIL_FIXED_POINT_SCALE,
                irrecoverable_cost_wei: spec.trail_config.score_cost_reference_wei,
            }],
            &spec,
        );
        ...
        assert_eq!(trail_evidence_score_for_epoch(Epoch::new(1), 0, 0), 0);
        assert_eq!(trail_evidence_score_for_epoch(Epoch::new(1), 1, 0), 0);
        let credit = trail_credit_epoch_summaries()
            .into_iter()
        ...
        assert_eq!(
            ...
            TRAIL_FIXED_POINT_SCALE
                .saturating_sub(spec.trail_config.proposer_fee_ratio_scaled)
        );
        reset_trail_evidence_runtime();
    }

    #[test]
    fn trail_fee_settlement_uses_committed_tx_fee_budget() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        ...
        let settlement = trail_fee_settlement_epoch_summaries()
            .into_iter()
            .find(|summary| summary.epoch == Epoch::new(0))
    ...
    #[test]
    fn trail_path_padding_does_not_increase_total_contribution() {
        let spec = trail_enabled_spec_for_tests();
        let short_total = trail_path_relay_contributions(&[0, 1, 3], &spec)
            .into_iter()
            .map(|(_, contribution)| contribution)
            .sum::<u64>();
        let padded_total = trail_path_relay_contributions(&[0, 1, 2, 3], &spec)
            .into_iter()
            .map(|(_, contribution)| contribution)
            .sum::<u64>();
    ...
    #[test]
    fn trail_score_credit_is_backed_by_irrecoverable_cost() {
        let reference = 42_000;
        assert_eq!(trail_transaction_credit_weight_scaled(0, reference), 0);
        assert_eq!(
            trail_transaction_credit_weight_scaled(reference / 2, reference),
            TRAIL_FIXED_POINT_SCALE / 2
        );
        assert_eq!(
            trail_transaction_credit_weight_scaled(reference * 2, reference),
    ...
    #[test]
    fn trail_distinct_txs_with_same_receiver_are_not_duplicates() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        let outcomes = record_trail_tx_gossip_metadata_evidence(
        ...
        assert!(
            outcomes
                .iter()
                .all(|outcome| matches!(outcome, TrailEvidenceRecordOutcome::Valid { .. }))
        );
        let summary = trail_evidence_epoch_summary(Epoch::new(0), &spec);
    ...
    #[test]
    fn trail_duplicate_tx_only_accepts_one_path() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        let tx_hash =
        ...
        assert!(matches!(
            ...
            [
                TrailEvidenceRecordOutcome::Valid { .. },
                TrailEvidenceRecordOutcome::DuplicateTransaction { .. }
            ]
        ));
        let summary = trail_evidence_epoch_summary(Epoch::new(0), &spec);
    ...
    #[test]
    fn trail_duplicate_tx_across_epochs_is_not_counted_again() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        let tx_hash =
        ...
        let first = record_trail_tx_gossip_metadata_evidence(
            Epoch::new(4),
            3,
            vec![TrailPathEvidence {
                tx_hash: tx_hash.clone(),
        ...
        let second = record_trail_tx_gossip_metadata_evidence(
            Epoch::new(5),
            3,
            vec![TrailPathEvidence {
                tx_hash,
        ...
        assert_eq!(
            trail_evidence_epoch_summary(Epoch::new(4), &spec).valid_paths,
            1
        );
        assert_eq!(
            trail_evidence_epoch_summary(Epoch::new(5), &spec).valid_paths,
            0
        );
        assert_eq!(
            trail_evidence_epoch_summary(Epoch::new(5), &spec).duplicate_receiver_proofs,
            1
        );
        reset_trail_evidence_runtime();
    }
    ...
    #[test]
    fn trail_no_evidence_score_stays_zero() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();

        let settled = trail_settle_scores_through_epoch(
            Epoch::new(0),
        ...
        assert_eq!(trail_evidence_score_for_epoch(Epoch::new(2), 0, 1), 0);
        reset_trail_evidence_runtime();
    }

    #[test]
    fn trail_saturation_has_diminishing_returns() {
        let stake = 32_000_000_000;
        let total = 128_000_000_000;
        let k = TRAIL_FIXED_POINT_SCALE;
        let one =
            trail_saturate_contribution_scaled(TRAIL_FIXED_POINT_SCALE, stake, total, k);
        let two = trail_saturate_contribution_scaled(
            TRAIL_FIXED_POINT_SCALE.saturating_mul(2),
            stake,
            total,
            k,
        );
        let four = trail_saturate_contribution_scaled(
            TRAIL_FIXED_POINT_SCALE.saturating_mul(4),
            stake,
            total,
            k,
        );
    ...
    #[test]
    fn trail_small_stake_repeated_contribution_is_not_linear() {
        let total = 128_000_000_000;
        let k = TRAIL_FIXED_POINT_SCALE;
        let small_once = trail_saturate_contribution_scaled(
            TRAIL_FIXED_POINT_SCALE,
            16_000_000_000,
            total,
            k,
        );
        let small_many = trail_saturate_contribution_scaled(
            TRAIL_FIXED_POINT_SCALE.saturating_mul(4),
            16_000_000_000,
            total,
            k,
    ...
    #[test]
    fn trail_score_settlement_waits_for_finalized_epoch() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        let outcomes = record_trail_tx_gossip_metadata_evidence(
        ...
        assert!(
            trail_settle_scores_through_epoch(
                Epoch::new(0),
                &[
                    (0, 32_000_000_000),
                    (1, 32_000_000_000),
    ...
    #[test]
    fn trail_selection_uses_latest_settled_score_before_next_scored_epoch() {
        let _guard = TRAIL_TEST_LOCK.lock().unwrap();
        reset_trail_evidence_runtime();
        let spec = trail_enabled_spec_for_tests();
        let outcomes = record_trail_tx_gossip_metadata_evidence(
        ...
        assert!(
            trail_evidence_score_for_epoch(Epoch::new(2), 1, 1) > 0,
            "settled score should affect the first eligible proposer epoch"
        );
        assert!(
            trail_evidence_score_for_epoch(Epoch::new(3), 1, 1) > 0,
