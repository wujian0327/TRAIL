...
pub struct TrailInlineEvidenceRecord<E: EthSpec> {
    pub tx_hash: Hash256,
    pub epoch: u64,
...
pub struct BeaconBlockBody<E: EthSpec, Payload: AbstractExecPayload<E> = FullPayload<E>> {
    ...
    /// Devnet-only TRAIL commitment. This field intentionally participates in SSZ and
    /// tree-hash roots in the custom fork.
    pub trail_evidence_root: Hash256,
    /// Prompt 25 bounded block-inline TRAIL path records. These records are committed by the
    /// block body root and let block processing avoid external evidence sidecar availability for
    /// the common path.
    pub trail_evidence_records:
...
impl<E: EthSpec> BeaconBlockBodyBellatrix<E, FullPayload<E>> {
    pub fn clone_as_blinded(&self) -> BeaconBlockBodyBellatrix<E, BlindedPayload<E>> {
        ...
        BeaconBlockBodyBellatrix {
            ...
            graffiti: *graffiti,
            trail_evidence_root: *trail_evidence_root,
