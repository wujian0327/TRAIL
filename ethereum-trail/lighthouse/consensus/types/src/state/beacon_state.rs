...
static TRAIL_SELECTION_SCORE_EPOCH: LazyLock<metrics::Result<metrics::IntGaugeVec>> =
    LazyLock::new(|| {
        metrics::try_create_int_gauge_vec(
            "trail_selection_score_epoch",
            "TRAIL evidence epoch used for proposer selection by proposer epoch",
            &["slot", "proposer_epoch"],
        )
    });

static TRAIL_PROPOSER_WEIGHT_SCALED: LazyLock<metrics::Result<metrics::GaugeVec>> =
    LazyLock::new(|| {
        metrics::try_create_float_gauge_vec(
            "trail_proposer_weight_scaled",
            "Scaled TRAIL proposer weight used during proposer selection",
            &["slot", "proposer_epoch", "score_epoch", "validator_index"],
        )
    });

static TRAIL_PROPOSER_SCORE_SCALED: LazyLock<metrics::Result<metrics::IntGaugeVec>> =
    LazyLock::new(|| {
        metrics::try_create_int_gauge_vec(
            "trail_proposer_score_scaled",
            "Scaled TRAIL score used during proposer selection",
            &["slot", "proposer_epoch", "score_epoch", "validator_index"],
        )
    });

static TRAIL_SELECTED_PROPOSER: LazyLock<metrics::Result<metrics::IntGaugeVec>> =
    LazyLock::new(|| {
        metrics::try_create_int_gauge_vec(
            "trail_selected_proposer",
            "TRAIL selected proposer by proposer epoch and evidence score epoch",
            &["slot", "proposer_epoch", "score_epoch", "validator_index"],
        )
    });
...
impl<E: EthSpec> BeaconState<E> {
    ...
    fn compute_beacon_proposer_index_for_epoch(
        ...
        {
            self.compute_trail_proposer_index(slot, epoch, indices, seed, spec)
        } else {
            self.compute_proposer_index(indices, seed, spec)
        }
    }

    fn compute_trail_proposer_index(
        &self,
        slot: Slot,
        epoch: Epoch,
        indices: &[usize],
        ...
        let score_epoch = epoch
            .as_u64()
            .checked_sub(spec.trail_config.score_activation_delay_epochs());
