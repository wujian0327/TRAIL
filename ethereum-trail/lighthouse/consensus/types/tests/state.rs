...
static TRAIL_EVIDENCE_TEST_LOCK: Mutex<()> = Mutex::new(());
...
#[tokio::test]
async fn trail_skeleton_preserves_default_proposers_and_roots() {
    type E = MinimalEthSpec;

    let spec = E::default_spec();
    let mut trail_spec = spec.clone();
    trail_spec.trail_config = TrailConfig::devnet_enabled_at(Epoch::new(0));

    let state = build_state::<E>((E::slots_per_epoch() as usize).mul(2)).await;
    let epoch = state.current_epoch();

    assert!(trail_spec.is_trail_enabled_at_epoch(epoch));
    assert_eq!(
        trail_spec
            .trail_config
            .zero_score_state_skeleton(epoch, state.validators().len()),
        Some(TrailStateSkeleton::zero_scores(
            epoch,
    ...
    let trail_zero_score_proposers = state
        .get_beacon_proposer_indices(epoch, &trail_spec)
        .expect("trail zero-score proposer duties should compute");
    assert_eq!(default_proposers, trail_zero_score_proposers);
    ...
    let mut trail_state = state.clone();
    let trail_state_root = trail_state.canonical_root().unwrap();
    let trail_latest_block_root = trail_state.get_latest_block_root(trail_state_root);

    assert_eq!(default_state_root, trail_state_root);
    assert_eq!(default_latest_block_root, trail_latest_block_root);
}

#[test]
fn trail_block_body_evidence_root_changes_body_root() {
    type E = MinimalEthSpec;

    let spec = E::default_spec();
    let mut block = BeaconBlock::<E>::empty(&spec);
    let zero_body_root = block.body_root();
    assert!(block.body().trail_evidence_root().is_zero());

    *block.body_mut().trail_evidence_root_mut() = Hash256::repeat_byte(0x42);

    assert_eq!(
        *block.body().trail_evidence_root(),
...
#[test]
fn trail_inline_evidence_record_changes_body_root() {
    type E = MinimalEthSpec;

    let spec = E::default_spec();
    let mut block = BeaconBlock::<E>::empty(&spec);
    let zero_body_root = block.body_root();
    assert!(block.body().trail_evidence_records().is_empty());

    block
        .body_mut()
        .trail_evidence_records_mut()
        .push(TrailInlineEvidenceRecord {
    ...
    assert_eq!(block.body().trail_evidence_records().len(), 1);
    assert_ne!(zero_body_root, block.body_root());
}

#[tokio::test]
async fn trail_zero_score_preserves_default_proposer_duties() {
    type E = MinimalEthSpec;

    let spec = E::default_spec();
    let mut trail_spec = spec.clone();
    trail_spec.trail_config = TrailConfig::devnet_enabled_at(Epoch::new(0));
    trail_spec.trail_config.score_fixture = vec![
        TrailScoreFixture {
            validator_index: 1,
            score_scaled: 0,
        },
...
#[tokio::test]
async fn trail_fixture_changes_proposer_duties_deterministically() {
    type E = MinimalEthSpec;

    let spec = E::default_spec();
    let mut trail_spec = spec.clone();
    trail_spec.trail_config = TrailConfig::devnet_enabled_at(Epoch::new(0));
    trail_spec.trail_config.eta_scaled = TRAIL_FIXED_POINT_SCALE;
    trail_spec.trail_config.bonus_cap_scaled = TRAIL_FIXED_POINT_SCALE;
    ...
    for slot in epoch.slot_iter(E::slots_per_epoch()) {
        ...
        trail_spec.trail_config.score_fixture = vec![TrailScoreFixture {
        ...
        let trail_proposer = state
            .get_beacon_proposer_index(slot, &trail_spec)
            .unwrap();
        if default_proposer != trail_proposer {
            changed_slot = Some((slot, first_candidate, default_proposer, trail_proposer));
            break;
        }
    ...
    let (slot, boosted_validator, default_proposer, trail_proposer) =
        changed_slot.expect("fixture should change at least one proposer duty");
    assert_eq!(trail_proposer, boosted_validator);
    assert_ne!(default_proposer, trail_proposer);
    assert_eq!(
        state
            .get_beacon_proposer_index(slot, &trail_spec)
            .unwrap(),
        trail_proposer,
        "same state/slot/seed should produce the same TRAIL proposer"
    );
}

#[test]
fn trail_bonus_cap_bound() {
    let mut config = TrailConfig::devnet_enabled_at(Epoch::new(0));
    config.eta_scaled = TRAIL_FIXED_POINT_SCALE / 4;
    config.bonus_cap_scaled = TRAIL_FIXED_POINT_SCALE;
    config.score_fixture = vec![TrailScoreFixture {
        validator_index: 0,
        score_scaled: TRAIL_FIXED_POINT_SCALE * 10,
    }];
...
#[test]
fn trail_proposer_bonus_uses_damped_score_over_stake_share() {
    let mut config = TrailConfig::devnet_enabled_at(Epoch::new(0));
    config.eta_scaled = TRAIL_FIXED_POINT_SCALE;
    config.bonus_cap_scaled = TRAIL_FIXED_POINT_SCALE;
    config.score_fixture = vec![
        ...
        TrailScoreFixture {
            validator_index: 1,
            score_scaled: TRAIL_FIXED_POINT_SCALE * 4,
        },
    ];
...
#[test]
fn trail_bonus_denominator_excludes_inactive_validators() {
    let mut config = TrailConfig::devnet_enabled_at(Epoch::new(0));
    config.score_fixture = vec![
        TrailScoreFixture {
            validator_index: 0,
        ...
        TrailScoreFixture {
            validator_index: 2,
            score_scaled: TRAIL_FIXED_POINT_SCALE * 100,
        },
    ];
    ...
    assert_eq!(active_score_total, TRAIL_FIXED_POINT_SCALE);
    assert!(active_set_bonus > registry_wide_bonus);
}

#[test]
fn trail_frozen_v1_matches_shared_golden_vectors() {
        ...
        .expect("credit weight vectors should be an array")
    {
        ...
        let actual = trail_transaction_credit_weight_scaled(
            (irrecoverable_cost * 1_000.0).round() as u64,
            (reference_cost * 1_000.0).round() as u64,
        );
        assert_eq!(
            actual,
            (expected * TRAIL_FIXED_POINT_SCALE as f64) as u64
        );
    }

    let config = TrailConfig::devnet_enabled_at(Epoch::new(0));
        ...
        .expect("bonus vectors should be an array")
    {
        ...
        let actual = config
            .concave_bonus_scaled((ratio * TRAIL_FIXED_POINT_SCALE as f64).round() as u64);
        let expected_scaled = (expected * TRAIL_FIXED_POINT_SCALE as f64).round() as u64;
        assert!(actual.abs_diff(expected_scaled) <= 1);
    }
}

#[test]
fn trail_unfinalized_path_score_does_not_affect_proposer_weight() {
    let _guard = TRAIL_EVIDENCE_TEST_LOCK.lock().unwrap();
    ...
    spec.trail_config.eta_scaled = TRAIL_FIXED_POINT_SCALE;
    spec.trail_config.evidence_finality_depth = 1;
    record_trail_tx_gossip_metadata_evidence(
        Epoch::new(0),
        2,
    ...
    assert_eq!(
        spec.trail_config.bonus_scaled_at_epoch(
            Epoch::new(1),
            balance,
            total_active_balance,
            1,
    ...
    let early_settled = trail_settle_scores_through_epoch(
        Epoch::new(0),
...
fn trail_test_graffiti(value: &str) -> Graffiti {
...
#[test]
fn trail_costless_graffiti_evidence_does_not_purchase_score() {
    let _guard = TRAIL_EVIDENCE_TEST_LOCK.lock().unwrap();
    reset_trail_evidence_runtime();
    ...
    let valid = trail_test_graffiti("TPS1:0,1,2,3");
    assert_eq!(
        decode_trail_graffiti_evidence(&valid, &spec).unwrap(),
        Some(vec![0, 1, 2, 3])
    );
    let outcome = record_trail_graffiti_evidence(Epoch::new(0), 42, &valid, &spec);
    assert!(matches!(
        outcome,
        TrailEvidenceRecordOutcome::Valid { receiver: 3, .. }
    ));
...
#[test]
fn trail_credit_ledger_records_valid_paths_only() {
    let _guard = TRAIL_EVIDENCE_TEST_LOCK.lock().unwrap();
    reset_trail_evidence_runtime();

    let mut spec = MinimalEthSpec::default_spec();
    spec.trail_config = TrailConfig::devnet_enabled_at(Epoch::new(0));
    spec.trail_config.reward_settlement_depth = 2;
    spec.trail_config.proposer_fee_ratio_scaled = TRAIL_FIXED_POINT_SCALE / 2;

    let valid = trail_test_graffiti("TPS1:0,1,2,3");
    let outcome = record_trail_graffiti_evidence(Epoch::new(0), 9, &valid, &spec);
    assert!(matches!(
        outcome,
        TrailEvidenceRecordOutcome::Valid { .. }
    ));

    let summaries = trail_credit_epoch_summaries();
    ...
    assert_eq!(summary.total_budget_scaled, TRAIL_FIXED_POINT_SCALE);
    assert_eq!(
    ...
    assert_eq!(
        ...
        vec![TrailValidatorCredit {
            validator_index: 9,
            credit_scaled: TRAIL_FIXED_POINT_SCALE / 2,
        }]
    );
    assert_eq!(
        summary.relay_credits,
        vec![
            TrailValidatorCredit {
                validator_index: 1,
    ...
    trail_settle_credits_through_epoch(Epoch::new(1));
    let summary = trail_credit_epoch_summaries()
        .pop()
        .expect("credit summary should exist");
    assert_eq!(summary.pending_records, 1);
    assert_eq!(summary.settled_records, 0);

    trail_settle_credits_through_epoch(Epoch::new(2));
    let summary = trail_credit_epoch_summaries()
        .pop()
        .expect("credit summary should exist");
    ...
    let csv = trail_credit_csv_snapshot();
    ...
    let fee_csv = trail_fee_settlement_csv_snapshot();
    ...
    let fee_summary = trail_fee_settlement_epoch_summaries()
        .pop()
...
#[test]
fn trail_invalid_and_duplicate_evidence_do_not_add_score() {
    let _guard = TRAIL_EVIDENCE_TEST_LOCK.lock().unwrap();
    reset_trail_evidence_runtime();
    ...
    let valid = trail_test_graffiti("TPS1:0,1,2,3");
    let duplicate_receiver = trail_test_graffiti("TPS1:0,1,2,3");
    let repeated_identity = trail_test_graffiti("TPS1:5,5");

    assert!(matches!(
        record_trail_graffiti_evidence(Epoch::new(0), 42, &valid, &spec),
        TrailEvidenceRecordOutcome::Valid { .. }
    ));
    assert!(matches!(
        record_trail_graffiti_evidence(Epoch::new(0), 42, &duplicate_receiver, &spec),
    ...
    assert!(matches!(
        record_trail_graffiti_evidence(Epoch::new(0), 42, &repeated_identity, &spec),
    ...
    let credit_summary = trail_credit_epoch_summaries()
        .pop()
    ...
    let csv = trail_evidence_csv_snapshot(&spec);
