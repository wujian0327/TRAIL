# TRAIL Ethereum Devnet Fork Spec
...
The goal is to define the smallest consensus boundary required for TRAIL to
...
- Treat missing or invalid path evidence as zero TRAIL score/reward, not as
...
These parameters must be identical for all consensus clients in a TRAIL
devnet.

| Parameter | Type | Suggested devnet default | Consensus? | Meaning |
|---|---:|---:|---|---|
| `trail_fork_epoch` | `Epoch` | disabled unless explicitly set | Yes | First epoch where TRAIL state fields and rules are active. |
...
| `max_path_evidence_bytes` | `uint64` | `65536` bytes per block | Yes | Maximum serialized TRAIL evidence bytes accepted for a block/sidecar. |
| `max_paths_per_block` | `uint64` | `1024` | Yes | Maximum path records processed for one block. |
| `proposer_fee_ratio` | fixed-point decimal, scaled `uint64` | `0.5` | Yes, if reward ledger is enabled | Fraction of TRAIL reward budget retained by proposer before relay sharing. |
...
All fields in this section are consensus fields once `trail_fork_epoch` is
...
|---|---|---|---|
| `trail_config` | `TrailConfig` or fork config constants | Yes if stored in state; otherwise genesis/fork config | Parameters used by all deterministic TRAIL rules. |
| `trail_score_epoch` | `Epoch` | Yes | Last epoch for which propagation scores were computed. |
| `trail_raw_score_snapshot` | `List[uint64, VALIDATOR_REGISTRY_LIMIT]` | Yes | Raw/saturated propagation score per validator before normalization. |
| `trail_normalized_score_snapshot` | `List[uint64, VALIDATOR_REGISTRY_LIMIT]` | Yes | Activated EMA score root used by the target proposer epoch. |
| `trail_frozen_stake_snapshot` | `List[Gwei, VALIDATOR_REGISTRY_LIMIT]` | Yes | Economic stake snapshot used to compute an epoch's proposer weights. |
| `trail_proposer_weight_snapshot` | `List[uint64, VALIDATOR_REGISTRY_LIMIT]` | Yes | Frozen proposer-sampling weights for the active/future epoch. |
| `trail_pending_relay_settlements` | `List[TrailSettlement, MAX_PENDING_TRAIL_SETTLEMENTS]` | Yes, if relay ledger is enabled | Deterministic queue of finalized relay/proposer credit deltas. |
| `trail_path_evidence_accumulator` | `Root` | Yes, if evidence is accumulated in state | Optional root of accepted/ignored evidence summaries for auditability. |
| `trail_latest_evidence_roots` | `Vector[Root, EPOCHS_PER_HISTORICAL_VECTOR]` | Yes, if committed sidecars are used | Per-epoch rolling roots of path evidence commitments. |

`trail_frozen_stake_snapshot` must not be updated intra-epoch. It is copied
...
```text
container TrailConfig:
    trail_fork_epoch: Epoch
    eta_scaled: uint64
...
container TrailPathRecord:
    tx_root: Root
    epoch: Epoch
    priority_fee_wei: uint64
    irrecoverable_cost_wei: uint64
    validator_path: List[ValidatorIndex, MAX_TRAIL_PATH_LEN]
    aggregate_signature: BLSSignature

container TrailEvidenceSidecar:
    slot: Slot
    block_root: Root
    proposer_index: ValidatorIndex
    records: List[TrailPathRecord, MAX_TRAIL_PATHS_PER_BLOCK]

container TrailSettlement:
...
Add TRAIL path evidence or a compact evidence commitment directly to the
beacon block body.
...
### Option B: Committed TRAIL Evidence Sidecar

Add a `trail_evidence_root` commitment to the beacon block body and carry
the full `TrailEvidenceSidecar` separately.
...
Carry TRAIL path evidence as Ethereum transaction calldata, as in Prompt 3
...
2. Prompt 11: add a committed evidence root. For the first implementation, the
   block body may carry only `trail_evidence_root`; a devnet-only sidecar
   carries full `TrailEvidenceSidecar`.
...
final encoding. A real TRAIL fork should replace this with a committed
...
`TRAIL_CONFIG.SIDECAR_FIXTURES` entry maps that tag to the full fixture path
...
carrier. Custom geth can attach an optional `TRAIL` metadata vector to
...
does not affect EVM execution. When TRAIL is enabled, direct transaction
...
`trail_tx_metadata_*` metrics demonstrate transaction-gossip carriage only;
...
public-key registry maps `validator_index` to TRAIL relay BLS pubkey. With
...
- Before `trail_fork_epoch`, all TRAIL state fields are absent or
  ignored, and upstream Ethereum proposer selection is used.
- At `trail_fork_epoch`, all nodes initialize TRAIL state fields from
  the same genesis/fork config.
- A node without this fork will compute different state roots or proposer
  duties once TRAIL consensus fields/rules are active.
...
- During epoch `e`, proposer duties use only `trail_proposer_weight_snapshot`
  for epoch `e`.
...
- TRAIL propagation score does not increase finality voting power;
...
- settlements are stored in `trail_pending_relay_settlements`;
...
`trail_pending_relay_settlements` field yet.
...
baseline and a TRAIL devnet both reached `head_slot=161`, produced 161
blocks with no missing slots, and finalized to epoch 3. Validators `0..3`
proposed `4/161` blocks in the baseline and `31/161` blocks in the TRAIL
run. The TRAIL run observed `valid_paths=21`, `score_sum=84000000000`,
...
- TRAIL fork activation config, if stored in state.
- TRAIL score snapshots.
...
- Any block body field such as `trail_evidence_root`.
...
## Why Mixed Stock And TRAIL Nodes Fork

Once TRAIL proposer selection is active, a TRAIL node computes proposer
duties from:

```text
trail_proposer_weight_snapshot
```
...
- a validator client attached to a TRAIL beacon node may sign a block for a
...
understand the fork cannot validate those roots. Therefore a TRAIL consensus
...
2. Add tests that zero TRAIL score equals ordinary PoS.
...
- Finality voting: TRAIL score does not alter attestation weight.
- Mixed stock/custom negative test: stock client rejects or diverges once
  TRAIL proposer selection or SSZ fields are active.
