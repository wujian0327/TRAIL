#!/usr/bin/env python3
"""Generate Kurtosis ethereum-package args for TRAIL devnet experiments."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any, Dict, List


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PUBLIC_REGISTRY = ROOT / "results" / "processed" / "trail_relay_key_registry.json"
DEFAULT_PRIVATE_REGISTRY = ROOT / "results" / "raw" / "trail_relay_keys_private.json"
FROZEN_PROFILE = json.loads(
    (ROOT / "experiments" / "configs" / "protocol_frozen_v1.yaml").read_text()
)
DEVNET_PROFILE = FROZEN_PROFILE["devnet_fixed_point"]


def compact_json(value: Dict[str, Any]) -> str:
    return json.dumps(value, separators=(",", ":"))


def q(value: str) -> str:
    return "'" + value + "'"


def load_json(path: Path) -> Dict[str, Any]:
    return json.loads(path.read_text())


def el_rpc_registry(count: int) -> Dict[str, Any]:
    return {
        "endpoints": [
            {
                "cl_service_name": f"cl-{participant_service_number(index, count)}-lighthouse-geth",
                "el_rpc_url": f"http://el-{participant_service_number(index, count)}-geth-lighthouse:8545",
            }
            for index in range(count)
        ]
    }


def participant_service_number(index: int, count: int) -> str:
    if count >= 10:
        return f"{index + 1:02d}"
    return str(index + 1)


def write_args(args: argparse.Namespace) -> None:
    lines: List[str] = []
    trail_features = args.mode != "baseline"
    fee_escrow_enabled = args.mode in ("fee_only", "trail") and not args.disable_fee_settlement
    scale = int(DEVNET_PROFILE["scale"])

    lines.extend(
        [
            "participants:",
            "  - el_type: geth",
            "    el_image: trail/geth:dev",
            "    el_extra_params:",
            (
                '      - "--http.api=admin,debug,eth,net,web3,txpool,trail"'
                if trail_features
                else '      - "--http.api=admin,debug,eth,net,web3,txpool"'
            ),
            '      - "--nodiscover"',
            f'      - "--maxpeers={args.maxpeers}"',
            '      - "--bootnodes="',
        ]
    )

    if trail_features:
        private_registry = load_json(args.private_registry)
        public_registry = load_json(args.public_registry)
        lines.extend(
            [
                "    el_extra_env_vars:",
                f'      TRAIL_CHAIN_ID: "{args.chain_id}"',
                '      TRAIL_RELAY_EPOCH: "0"',
                '      TRAIL_DYNAMIC_RELAY_EPOCH: "1"',
                f'      TRAIL_SECONDS_PER_SLOT: "{DEVNET_PROFILE["seconds_per_slot"]}"',
                f'      TRAIL_SLOTS_PER_EPOCH: "{DEVNET_PROFILE["slots_per_epoch"]}"',
                f'      TRAIL_FEE_ESCROW: "{1 if fee_escrow_enabled else 0}"',
                f'      TRAIL_SETTLEMENT_MUTATION: "{1 if fee_escrow_enabled else 0}"',
                f"      TRAIL_RELAY_PRIVATE_REGISTRY_JSON: {q(compact_json(private_registry))}",
            ]
        )
    lines.extend(
        [
            "    cl_type: lighthouse",
            "    cl_image: trail/lighthouse:dev",
        ]
    )
    if trail_features:
        public_registry = load_json(args.public_registry)
        rpc_registry = el_rpc_registry(args.count)
        eta_scaled = args.eta_scaled if args.mode in ("bonus_only", "trail") else 0
        lines.extend(
            [
                "    cl_extra_env_vars:",
                f"      TRAIL_RELAY_PUBLIC_REGISTRY_JSON: {q(compact_json(public_registry))}",
                f"      TRAIL_EL_RPC_REGISTRY_JSON: {q(compact_json(rpc_registry))}",
                '      TRAIL_TX_EVIDENCE_GRAFFITI_COMMITMENT: "1"',
                '      TRAIL_FORK_EPOCH: "0"',
                f'      TRAIL_ETA_SCALED: "{eta_scaled}"',
                f'      TRAIL_EVIDENCE_FINALITY_DEPTH: "{DEVNET_PROFILE["evidence_finality_depth"]}"',
                f'      TRAIL_SCORE_ACTIVATION_DELAY_EPOCHS: "{DEVNET_PROFILE["score_activation_delay_epochs"]}"',
            ]
        )
    lines.extend(
        [
            f"    count: {args.count}",
            f"    validator_count: {args.validator_count}",
            "    prometheus_config:",
            "      scrape_interval: 15s",
            "      labels:",
            f"        experiment: {args.label}",
            "",
            "network_params:",
            "  network: kurtosis",
            f'  network_id: "{args.chain_id}"',
            "  seconds_per_slot: 3",
            "  slot_duration_ms: 3000",
            "  genesis_delay: 20",
            "  genesis_gaslimit: 60000000",
            f"  num_validator_keys_per_node: {args.validator_count}",
            "  validator_balance: 32",
            "  fulu_fork_epoch: 18446744073709551615",
            "  preset: minimal",
        ]
    )
    if trail_features:
        eta_scaled = args.eta_scaled if args.mode in ("bonus_only", "trail") else 0
        lines.extend(
            [
                "",
                "ethereum_genesis_generator_params:",
                "  extra_env:",
                "    TRAIL_CONFIG_YAML: |",
                "      TRAIL_CONFIG:",
                '        TRAIL_FORK_EPOCH: "0"',
                f'        ETA_SCALED: "{eta_scaled}"',
                f'        BONUS_CAP_SCALED: "{scale}"',
                f'        SCORE_EMA_BETA_SCALED: "{int(float(FROZEN_PROFILE["beta"]) * scale)}"',
                f'        SCORE_SATURATION_K_SCALED: "{int(float(FROZEN_PROFILE["saturation_k"]) * scale)}"',
                f'        SCORE_TARGET_DEPTH: "{FROZEN_PROFILE["target_depth"]}"',
                f'        SCORE_COST_REFERENCE_WEI: "{DEVNET_PROFILE["score_cost_reference_wei"]}"',
                f'        SCORE_FLOOR_KAPPA_SCALED: "{DEVNET_PROFILE["score_floor_kappa_scaled"]}"',
                f'        BONUS_ZETA_SCALED: "{DEVNET_PROFILE["bonus_zeta_scaled"]}"',
                f'        EVIDENCE_FINALITY_DEPTH: "{DEVNET_PROFILE["evidence_finality_depth"]}"',
                f'        SCORE_ACTIVATION_DELAY_EPOCHS: "{DEVNET_PROFILE["score_activation_delay_epochs"]}"',
                f'        MAX_PATH_EVIDENCE_LEN: "{DEVNET_PROFILE["max_path_evidence_len"]}"',
                f'        MAX_PATHS_PER_BLOCK: "{DEVNET_PROFILE["max_paths_per_block"]}"',
                f'        EVIDENCE_WORK_LIMIT: "{DEVNET_PROFILE["evidence_work_limit"]}"',
                f'        CHALLENGE_WORK_LIMIT: "{DEVNET_PROFILE["challenge_work_limit"]}"',
            ]
        )
    if args.enable_observability:
        lines.extend(
            [
                "",
                "additional_services:",
                "  - prometheus",
                "  - grafana",
                "",
                "prometheus_params:",
                '  storage_tsdb_retention_time: "1d"',
                '  storage_tsdb_retention_size: "512MB"',
                "",
                "ethereum_metrics_exporter_enabled: true",
            ]
        )
    else:
        lines.extend(
            [
                "",
                "additional_services: []",
                "ethereum_metrics_exporter_enabled: false",
            ]
        )
    lines.extend(
        [
            "global_log_level: info",
            "",
        ]
    )
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text("\n".join(lines))
    print(f"wrote {args.out}")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--mode",
        choices=["baseline", "pathobs", "fee_only", "bonus_only", "trail"],
        required=True,
    )
    parser.add_argument("--count", type=int, default=8)
    parser.add_argument("--validator-count", type=int, default=16)
    parser.add_argument("--chain-id", type=int, default=7_032_030)
    parser.add_argument("--maxpeers", type=int, default=8)
    parser.add_argument("--eta-scaled", type=int, default=500_000_000)
    parser.add_argument("--disable-fee-settlement", action="store_true")
    parser.add_argument("--enable-observability", action="store_true")
    parser.add_argument("--label", default="trail-devnet-8node-overhead")
    parser.add_argument("--public-registry", type=Path, default=DEFAULT_PUBLIC_REGISTRY)
    parser.add_argument("--private-registry", type=Path, default=DEFAULT_PRIVATE_REGISTRY)
    parser.add_argument("--out", type=Path, required=True)
    write_args(parser.parse_args())


if __name__ == "__main__":
    main()
