...
const DEFAULT_SEED: &str = "trail-prompt14-relay-key-v1";
...
fn main() {
    ...
    let public_out = parse_arg(
        &args,
        "--public-out",
        "results/processed/trail_relay_key_registry.json",
    );
    let private_out = parse_arg(
        &args,
        "--private-out",
        "results/raw/trail_relay_keys_private.json",
    );
    ...
    let public_registry = PublicRegistry {
        version: 1,
        chain_id,
        signing_domain: "TRAIL_TX_PATH_V1",
        note: "Public TRAIL relay BLS registry for Prompt 14 devnet verification. These keys are independent from Ethereum validator consensus keys.",
    ...
    let private_keys = PrivateKeyFile {
        ...
        chain_id,
        signing_domain: "TRAIL_TX_PATH_V1",
        note: "Devnet-only TRAIL relay private keys. Mount exactly one private key into the matching custom execution client; do not mount Ethereum validator keys.",
