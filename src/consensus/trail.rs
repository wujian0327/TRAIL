...
impl Default for TrailConfig {
    fn default() -> Self {
        serde_json::from_str(FROZEN_V1_CONFIG_JSON)
            .expect("embedded frozen-v1 TRAIL config must be valid JSON")
    }
}
...
impl TrailConsensus {
    pub fn new(base_reward: f64, config: TrailConfig) -> Result<Self, String> {
        config.validate()?;
        Ok(TrailConsensus {
            config,
            base_reward,
    ...
    fn select_with_frozen_weights(
        ...
        for validator in validators {
            ...
            if accumulated_weight >= random_value {
                info!(
                    "TRAIL revised proposer {} elected with frozen weight {:.6}",
...
impl Consensus for TrailConsensus {
    ...
    fn metrics_snapshot(&self) -> ConsensusMetricsSnapshot {
        ConsensusMetricsSnapshot {
            trail_depth: Some(self.config.target_depth),
            trail_beta: Some(self.config.beta),
            trail_eta: Some(self.config.eta),
            trail_bonus_cap: Some(self.config.bonus_cap),
            trail_saturation_k: Some(self.config.saturation_k),
            trail_proposer_fee_ratio: Some(self.config.proposer_fee_ratio),
            trail_score_floor_kappa: Some(self.config.score_floor_kappa),
            trail_bonus_zeta: Some(self.config.bonus_zeta),
            trail_score_cost_reference: Some(self.config.score_cost_reference),
            trail_active_score_epoch: self.active_score_epoch,
            trail_latest_score_epoch: self.next_epoch.checked_sub(1),
    ...
    fn distribute_rewards(
        ...
        info!(
            "TRAIL revised rewards: block={} proposer={:.6} relay={:.6} burned={:.6}",
...
mod tests {
    ...
    fn path_budget_sums_to_budget() {
        ...
        for path_length in 2..12 {
            let budget = TrailConsensus::path_budget_for_depth(depth, path_length);
    ...
    fn frozen_v1_golden_vectors_match_rust_formulas() {
        ...
        for vector in vectors.credit_weights {
            let actual = TrailConsensus::transaction_credit_weight(
        ...
        for vector in vectors.bonuses {
            let actual = TrailConsensus::propagation_bonus(
        ...
        let damped = TrailConsensus::damped_score_for_active_set(
            &scores,
            &validators,
        ...
        for index in 0..weight.stakes.len() {
            let bonus = TrailConsensus::propagation_bonus(
    ...
    fn path_budget_decreases_with_length() {
        let depth = 4;
        let mut previous = TrailConsensus::path_budget_for_depth(depth, 2);
        for path_length in 3..16 {
            let current = TrailConsensus::path_budget_for_depth(depth, path_length);
    ...
    fn lambda_times_one_plus_r_is_one() {
        for depth in 1..20 {
            let lambda = TrailConsensus::lambda_for_depth(depth);
            let r = TrailConsensus::r_for_depth(depth);
    ...
    fn transaction_credit_weight_is_cost_backed_and_capped() {
        let reference = 10.0;
        assert_eq!(
            TrailConsensus::transaction_credit_weight(0.0, reference),
            0.0
        );
        assert!(
            (TrailConsensus::transaction_credit_weight(2.5, reference) - 0.25).abs() < 1e-12
        );
        assert_eq!(
            TrailConsensus::transaction_credit_weight(20.0, reference),
            1.0
        );
    }
    ...
    fn raw_contribution_is_weighted_by_irrecoverable_cost() {
        ...
        let consensus = TrailConsensus::new(1.0, config).unwrap();
    ...
    fn score_floor_dampens_small_absolute_score() {
        ...
        let mut consensus = TrailConsensus::new(1.0, test_config()).unwrap();
        consensus
    ...
    fn score_root_activates_only_after_fixed_delay() {
        ...
        let mut consensus = TrailConsensus::new(1.0, config).unwrap();
    ...
    fn proposer_weight_bound_holds_per_validator_and_coalition() {
        ...
        consensus.freeze_proposer_weights(&validators);
        let stake = TrailConsensus::normalized_stake(&validators);
    ...
    fn proposer_sequence_is_deterministic_for_seed_sequence() {
        ...
        let mut a = TrailConsensus::new(1.0, test_config()).unwrap();
        let mut b = TrailConsensus::new(1.0, test_config()).unwrap();
    ...
    fn fee_budget_balance_uses_gamma() {
        ...
        let miner = Wallet::new();
        let (block, validators) = block_with_path(&origin, &relay, &miner, 10.0);
        let consensus = TrailConsensus::new(1.0, test_config()).unwrap();
